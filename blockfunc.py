...
def _make_groups_tensor(inp, group_mantissa, group_size, group_direction):
    ...
    if group_direction == 0: # WI Mode, If kernel size is not 3, it will not work properly
        ...
        if inp_n.shape[1] % g_kernel != 0:
            inp_n = np.pad(inp_n, ((0,0),(0,g_kernel-inp_n.shape[0]%g_kernel),(0,0),(0,0)))
        # Transpose the array to match direction
        inp_n = np.transpose(inp_n, (2,3,1,0))
    # Code modified from https://stackoverflow.com/questions/42297115/numpy-split-cube-into-cubes/42298440#42298440
    elif group_direction == 2: # FX Mode
        # TODO : Prevent overflow caused by mapping next kernel map
        # Pad Array to have more values
        ...
        # Pad Array to have more values
        if inp_n.shape[2] % 3 != 0 or inp_n.shape[3] % 3 != 0:
            inp_n = np.pad(inp_n, ((0,0),(0,0),(0,3-inp_n.shape[0]%3),(0,3-inp_n.shape[1]%3)))
        # Reshape to corresponding size to manipulate easier
        inp_n = inp_n.reshape((inp_n.shape[0], 1, inp_n.shape[1], 1, inp_n.shape[2]//3, 3, inp_n.shape[3]//3, 3))
        # Transpose to have 3x3 structure is preserved
        inp_n = inp_n.transpose((2,0,4,6,5,7,1,3))
    elif group_direction == 4:
        # TODO : Prevent overflow caused by mapping next kernel map
        # Pad Array to have more values
        if inp_n.shape[1] % g_kernel != 0 or inp_n.shape[2] % 3 != 0 or inp_n.shape[3] % 3 != 0:
            inp_n = np.pad(inp_n, ((0,0),(0,0),(0,3-inp_n.shape[0]%3),(0,3-inp_n.shape[1]%3)))
        # Reshape to corresponding size to manipulate easier
        inp_n = inp_n.reshape((inp_n.shape[0], 1, inp_n.shape[1], 1, inp_n.shape[2]//3, 3, inp_n.shape[3]//3, 3))
        # Transpose to have 3x3 structure is preserved
        inp_n = inp_n.transpose((4,0,2,6,5,7,1,3))
    else:
    ...
    # IDEA : send shift code to back, maybe that's faster
    np.right_shift(e_, 23, out=e_)
    # Match shape to divisible to group size <- maybe this is not necessary because of pre-processing
    m_ = np.append(e_, np.zeros(group_size - e_.shape[0] % group_size, dtype=np.uint32))
    m_ = np.reshape(m_, (group_size, -1))
    # get the max value of each blocks
    m_ = np.amax(m_, axis=0)
    ...
    if group_direction == 0: # WI
        ...
        print(r.shape)
        r = r.reshape(inp_shape)
    elif group_direction == 3: # FY Mode
        r = r.transpose((1,6,0,7,2,4,3,5))
        r = r.reshape(inp_shape)
    elif group_direction == 4:
        r = r.transpose((1,6,2,7,0,4,3,5))
        r = r.reshape(inp_shape)
    else:
        raise ValueError("group_direction not supported")
